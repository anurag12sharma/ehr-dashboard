...
async function seedDatabase() {
  try {
    ...
    // Seed patients
    const patientEntries = fhirData.entry.filter(entry => 
    ...
    for (const entry of patientEntries) {
      ...
      console.log('\naddress typeof', typeof resource.address, '\n', resource.address);
    ...
    const appointments = appointmentData.entry.map(a => ({
      fhirId: a.resource.id,                 // <--- map FHIR id to schema's required fhirId field
      resourceType: a.resource.resourceType, // 'Appointment'
      status: a.resource.status,
      appointmentType: a.resource.appointmentType,
      reasonCode: a.resource.reasonCode,
      supportingInformation: a.resource.supportingInformation,
      start: a.resource.start,
      end: a.resource.end,
      minutesDuration: a.resource.minutesDuration,
      created: a.resource.created,
      participant: a.resource.participant,
      meta: a.resource.meta,
    }));

    await Appointment.insertMany(appointments);

    console.log(`✅ Seeded ${patientEntries.length} patients`);
